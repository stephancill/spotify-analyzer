...
function ready() {
    ...
    fetch(userEndpoint, GETRequestConfig).then(function (blob) {
        return blob.json();
    }).then(function (data) {
    ...
    function getPlaylists() {
        ...
        fetch(playlistsEndpoint, GETRequestConfig).then(function (blob) {
            return blob.json();
        }).then(function (data) {
            handlePlaylistResponse(data);
            recursiveGet(data, playlistsEndpoint, GETRequestConfig, 1, 20, handlePlaylistResponse, maxRequests = null, callback = function callback() {
                ...
                // Click listener
                document.getElementById("confirmSource").addEventListener("click", function () {
                    ...
                    if (value != "playlistURLOption") {
                        ...
                        if (endpoint === "__!__Invalid link__!_#") {
                            ...
                            fetch(endpoint, GETRequestConfig).then(function (blob) {
                                return blob.json();
                            }).then(function (data) {
...
function populateSongs(endpoint) {
    ...
    fetch(endpoint + ("?limit=" + limit + "&offset=" + requestsMade * limit), GETRequestConfig).then(function (blob) {
        return blob.json();
    }).then(function (data) {
        data.items.map(function (item) {
        ...
        recursiveGet(data, endpoint, GETRequestConfig, requestsMade + 1, limit, function (data) {
            data.items.map(function (item) {
                songs.push({ title: item.track.name, artist: item.track.artists[0].name });
            });
            updateSongCount(data.items);
...
function recursiveGet(responseData, endpoint, request, requestsMade, limit) {
    var action = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : function () {};
    var maxRequests = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : null;
    ...
    if (!(responseData.items.length < limit)) {
        fetch(endpoint + ("?limit=" + limit + "&offset=" + requestsMade * limit), request).then(function (blob) {
            return blob.json();
        }).then(function (data) {
            action(data);
...
function getSongTags() {
    if (lastFMProcessIndex < songs.length) {
        ...
        fetch(endpoint + params + appendix).then(function (blob) {
            return blob.json();
        }).then(function (data) {
...
function analyzeSongs() {
    ...
    var keysSorted = Object.keys(genreDemographics).sort(function (b, a) {
        return genreDemographics[a].count - genreDemographics[b].count;
    });
    var tabulated = [];
    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: '# of songs',
                data: data,
                backgroundColor: ['rgba(255, 99, 132, 0.2)', 'rgba(54, 162, 235, 0.2)', 'rgba(255, 206, 86, 0.2)', 'rgba(75, 192, 192, 0.2)', 'rgba(153, 102, 255, 0.2)', 'rgba(255, 159, 64, 0.2)', 'rgba(255, 60, 160, 0.2)'],
                borderColor: ['rgba(255,99,132,1)', 'rgba(54, 162, 235, 1)', 'rgba(255, 206, 86, 1)', 'rgba(75, 192, 192, 1)', 'rgba(153, 102, 255, 1)', 'rgba(255, 159, 64, 1)', 'rgba(255, 60, 160, 1)'],
                borderWidth: 1
            }]
        },
        options: {
